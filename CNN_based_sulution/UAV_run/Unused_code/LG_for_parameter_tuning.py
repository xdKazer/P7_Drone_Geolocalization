...
import csv, math, gc, random, cv2, torch, time, os
...
DEFAULT_HP = {
    # --- fusion of LG + shape + reproj ---
    "weight_LG_shape": 0.6,          # weight on LightGlue confidence
    "s_err": 2.0,              # reproj error scale in px
    "n_err_min": 80,           # min inliers before reproj error is trusted
    "alpha_err": 0.2,          # weight of reproj error term when used

    # --- EKF update gating ---
    "min_conf_for_ekf": 0.2,   # MIN_CONFIDENCE_FOR_EKF_UPDATE

    # --- measurement noise R (position, heading) ---
    "pos_base_std_px": 55.0,     # typical position std in px
    "pos_min_scale": 0.5,        # high-confidence scale
    "pos_max_scale": 2.0,        # low-confidence scale
    "heading_base_std_deg": 3.0, # typical heading std in deg
    "heading_min_scale": 0.5,    # high-confidence scale
    "heading_max_scale": 2.75,   # low-confidence scale

    # --- process noise Q (model uncertainty) ---
    "sigma_pos_proc": 3.0,       # px/√s baseline diffusion in x,y
    "sigma_speed": 0.5,          # px/√s speed random walk
    "sigma_phi_deg": 0.5,        # deg/√s heading random walk
    "sigma_b_phi_deg": 0.0025,   # deg/√s bias random walk
}

########################################################################################
# --------------------- Helpers and EKF Class -------------------------------------------
########################################################################################
def load_hp_from_env() -> dict:
    """
    Read hyperparameters from environment variables (if set)
    and override DEFAULT_HP accordingly.
    Environment variable names are UPPER_CASE, HP keys are lower_case.
    """
    hp = DEFAULT_HP.copy()

    env_to_hp = {
        # measurement R
        "POS_BASE_STD":         "pos_base_std_px",
        "POS_MIN_SCALE":        "pos_min_scale",
        "POS_MAX_SCALE":        "pos_max_scale",
        "HEADING_BASE_STD_DEG": "heading_base_std_deg",
        "HEADING_MIN_SCALE":    "heading_min_scale",
        "HEADING_MAX_SCALE":    "heading_max_scale",

        # EKF update gating
        "MIN_CONF_FOR_EKF":     "min_conf_for_ekf",

        # (optional later if you want)
        "WEIGHT_LG_SHAPE":      "weight_LG_shape",
        "S_ERR":                "s_err",
        "N_ERR_MIN":            "n_err_min",
        "ALPHA_ERR":            "alpha_err",
        # "SIGMA_POS_PROC":       "sigma_pos_proc",
        # "SIGMA_SPEED":          "sigma_speed",
        # "SIGMA_PHI_DEG":        "sigma_phi_deg",
        # "SIGMA_B_PHI_DEG":      "sigma_b_phi_deg",
    }

    for env_name, hp_key in env_to_hp.items():
        val = os.environ.get(env_name)
        if val is None:
            continue
        try:
            # n_err_min is logically int, others float – handle that if you add it
            if hp_key == "n_err_min":
                hp[hp_key] = int(float(val))
            else:
                hp[hp_key] = float(val)
        except ValueError:
            print(f"[warning] Could not parse env {env_name}={val!r} as number; ignoring.")
...
def get_confidence_meas(
    ...
    """
    Confidence with:
      - Weighted LG vs shape:
            c_prior = w_lg * c_lg + w_shape * c_shape
      - OPTIONAL small influence from reprojection error c_err
        when num_inliers >= HP["n_err_min"].
    """
    # basic guards
    if LG_conf <= 0 or num_inliers <= 0:
        return 0.0
    ...
    # normalize weights just in case sweeps give weird combos
    alpha = HP["weight_LG_shape"]

    c_prior = float(np.clip(alpha * c_lg + (1.0 - alpha) * c_shape, 0.0, 1.0))

    # --- if not enough inliers or bad error -> ignore reprojection term ---
    n_err_min = HP["n_err_min"]
    s_err     = HP["s_err"]
    alpha_err = HP["alpha_err"]
...
def get_shape_score(
    ...
    tau_aspect_ratio=0.20,            # tolerance for aspect ratio
...
"""
def main(hp: dict | None = None):
    global HP, MIN_CONFIDENCE_FOR_EKF_UPDATE
    global ekf, t_last, x_updated
    global CSV_FINAL_RESULT_PATH, SAT_LONG_LAT_INFO_DIR, SAT_DISPLAY_META, SAT_DIR, DRONE_INFO_DIR, DRONE_IMG_CLEAN, OUT_DIR_CLEAN, OUT_OVERALL_SAT_VIS_PATH, TILE_PT_DIR, SAT_DISPLAY_IMG

    # update HP for this run
    if hp is None:
        HP = DEFAULT_HP.copy()
    else:
        HP = DEFAULT_HP.copy()
        HP.update(hp)
    MIN_CONFIDENCE_FOR_EKF_UPDATE = HP["min_conf_for_ekf"]
    ...
    TILE_PT_DIR = DATASET_DIR / sat_number / f"{FEATURES}_features" / sat_number
    SAT_DISPLAY_IMG  = DATASET_DIR / sat_number / "satellite03_small.png"
    SAT_DISPLAY_META = SAT_DISPLAY_IMG.with_suffix(SAT_DISPLAY_IMG.suffix + ".json")
    TILE_WH_DIR = SAT_DIR / "a_tile_size.txt"  
    ...
    if OUT_DIR_CLEAN.exists() and OUT_DIR_CLEAN.is_dir():
    ...
    for i, img_path in enumerate(sorted(DRONE_IMG_CLEAN.iterdir())):
        ...
        if ekf is None: # first frame is skipped for EKF initialization
            ...
            Q0 = np.diag([
                HP["sigma_pos_proc"],                         # px/√s
                HP["sigma_speed"],                            # px/√s
                np.deg2rad(HP["sigma_phi_deg"]),              # rad/√s
                np.deg2rad(HP["sigma_b_phi_deg"]),            # rad/√s
            ])
        ...
        with torch.inference_mode():
            for i, p in enumerate(selected_tiles):
                ...
                shape_terms = np.full(4, np.nan, dtype=np.float64)
                K = None
                H = None
                is_convex_dlt = False
                H_ransac = None
        ...
        R = ekf.R_from_conf(
            pos_base_std=HP["pos_base_std_px"],
            heading_base_std_rad=np.deg2rad(HP["heading_base_std_deg"]),
            overall_conf=best_conf_overall,
            pos_min_scale=HP["pos_min_scale"],
            pos_max_scale=HP["pos_max_scale"],
            heading_min_scale=HP["heading_min_scale"],
            heading_max_scale=HP["heading_max_scale"],
        )
