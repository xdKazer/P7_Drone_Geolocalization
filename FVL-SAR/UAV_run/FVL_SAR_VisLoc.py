...
sat_number = "01"          #| "01" | "02" | "03" | "04" | "05" | "06" | "07" | "08" | "09" | "10"  | "11" |
...
def draw_point_old(img_bgr, pt_xy, color=(0,0,255), r=4):
    cv2.circle(img_bgr, (int(pt_xy[0]), int(pt_xy[1])), r, color, -1)

def draw_point(img_bgr, pt_xy, color=(0,0,255), r=4, thickness=4):
    cv2.drawMarker(img_bgr, (int(pt_xy[0]), int(pt_xy[1])), color, markerType=cv2.MARKER_TILTED_CROSS, markerSize=8, thickness=thickness)
...
for i, img_path in enumerate(sorted(DRONE_IMG_CLEAN.iterdir())):
    ...
    if drone_img in starting_drone_images: # this indicates a new sequence so ekf reset
        ...
        if visualisations_enabled and drone_img == starting_drone_images[0]:
    ...
    if visualisations_enabled: 
        ...
        for i in range(1):
            if i == -1: # individual overlay for this tile
